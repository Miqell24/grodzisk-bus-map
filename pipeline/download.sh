...
# ways, a railway corridor to a few hundred, so the caller passes its own floor
...
if [ ! -f data/osm/grodzisk.json ]; then
  ...
  for EP in "https://overpass-api.de/api/interpreter" \
    ...
    if curl -fsS --max-time 900 -o data/osm/grodzisk.json --data-urlencode "data=$Q" "$EP" \
       && ok_json "data/osm/grodzisk.json" 2000; then
      ok=1; break
    fi
  done
  [ "$ok" = 1 ] || { rm -f data/osm/grodzisk.json; echo "Overpass: all mirrors failed" >&2; exit 1; }
fi
...
if [ ! -f data/osm/grodzisk-rail.json ]; then
  ...
  for EP in "https://overpass-api.de/api/interpreter" \
    ...
    if curl -fsS --max-time 300 -o data/osm/grodzisk-rail.json --data-urlencode "data=$QT" "$EP" \
       && ok_json "data/osm/grodzisk-rail.json" 40; then
      ok=1; break
    fi
  done
  [ "$ok" = 1 ] || { rm -f data/osm/grodzisk-rail.json; echo "Overpass (rails): all mirrors failed" >&2; exit 1; }
fi
