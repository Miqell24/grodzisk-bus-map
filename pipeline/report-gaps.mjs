...
const feedInfo = (await readCsv(join(ROOT, 'data/gtfs-wkd/feed_info.txt')))[0] || {};
const routes = await readCsv(join(ROOT, 'data/gtfs-wkd/routes.txt'));
const routeToLine = new Map(routes.map((r) => [r.route_id, r.route_short_name]));
const shapeLines = new Map(); // shape_id -> Set(line)
for await (const t of iterCsv(join(ROOT, 'data/gtfs-wkd/trips.txt'))) {
...
const pts = new Map();
for await (const s of iterCsv(join(ROOT, 'data/gtfs-wkd/shapes.txt'))) {
...
const osm = JSON.parse(readFileSync(join(ROOT, 'data/osm/grodzisk.json'), 'utf8'));
const CELL = 250;
