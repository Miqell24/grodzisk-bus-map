...
// Grodzisk Mazowiecki and its county: TWO feeds, two modes — the GPA bus
// network (Związek Powiatowo-Gminny "Grodziskie Przewozy Autobusowe") on the
// road graph, and the WKD interurban railway on the rail graph. The WKD keeps
// its operator purple straight from routes.txt and takes the engine's metro
// treatment (wide ribbon, station discs, always-on names): it runs on its own
// right of way with proper stations, so the tram styling would undersell it.
// Usage: node pipeline/build.mjs [--all | lines...] [--tram all|WKD]
// Quirks: the GPA feed has no shapes.txt AND no direction_id — routes are
// matched from their stop sequences, and the direction is taken from the
// headsign (see the feed's dirKey).
...
// Service tracks stay excluded — this only reconnects mainline tunnels that OSM
// left as islands. Inert here (this region has no subway; the WKD tracks are
// mapped as one continuous railway=rail chain) and deliberately NOT extended to
// plain rail: the parallel PKP line 1 shares the extract, and welding rail
// stubs by proximity would bridge the two networks at Grodzisk and Pruszków.
...
// TWO feeds, TWO modes. The GPA feed is a vendor export (kiedyPrzyjedzie.pl)
// with two gaps the engine has to work around:
//   * no shapes.txt — every route is matched from its stop sequence;
//   * direction_id is present as a column but EMPTY on all 3884 trips, so the
//     usual line+direction split would collapse both directions of a line into
//     one representative and half of every one-way pair would go undrawn.
//     dirKey falls back to the headsign, which is the destination and therefore
//     a faithful stand-in for the direction. Lines with short-turn variants get
//     one rep per variant — more street coverage, which is what a poster map of
//     a county network wants.
const MODES = [{
  mode: 'bus', label: 'buses', osmFile: 'data/osm/grodzisk.json',
  graphMode: 'road', color: '#0059a9', colorDark: '#00294f',
  all: busAll, lines: busList.length ? busList : (busAll ? [] : ['17']),
  feeds: [
    {
      tag: 'gpa', dir: 'data/gtfs-gpa', mapKey: (sn) => sn, routeTypes: ['3'],
      dirKey: (t) => t.trip_headsign || '0',
    },
  ],
}];
// The rail slot: WKD (Warszawska Kolej Dojazdowa), the interurban railway from
// Grodzisk Mazowiecki Radońska to Warszawa Śródmieście with the Milanówek
// Grudów branch. It rides the 'tram' mode slot but answers to isMetroLine
// below, so it is drawn as a ribbon with station discs. routeTypes ['2'] also
// drops the feed's second route, "WKD ZKA" — the rail-replacement bus, which is
// route_type 3 and would otherwise be map-matched onto the railway graph.
const tramAll = tramLines.length === 1 && tramLines[0] === 'all';
if (tramLines.length) MODES.push({
  mode: 'tram', label: 'WKD', osmFile: 'data/osm/grodzisk-rail.json',
...
// Which rail keys get the metro treatment — the wide ribbon, full station
// discs and always-on station names. The sibling cities test for an M/U prefix;
// here the whole rail mode is the single WKD line, and it earns the treatment
// on its own merits (own right of way, 26 named stations, no street running).
const isMetroLine = (l) => l === 'WKD';
...
async function processMode(cfg) {
  ...
  // an all-metro mixed set must NOT fall back to the mode color — that is the
  // tram red, and a rail ribbon in tram red reads as a tram line. Purple =
  // "several rail lines". Unreachable here (the WKD is the whole rail mode);
  // kept so the color rules stay identical to the sibling cities.
  ...
  for (const feed of cfg.feeds) {
    ...
    for (const r of await readCsv(join(fdir, 'routes.txt'))) {
      ...
      } else if (cfg.mode === 'tram' && /^[0-9A-F]{6}$/i.test(r.route_color || '')) {
        // the feed ships the WKD's operator purple. Restricted to the rail mode
        // on purpose: the vendor also colors every GPA bus route (a rotating
        // palette of eight hues that says nothing about the network), and
        // honouring those would shred the navy bus stroke into confetti.
    ...
    for await (const t of iterCsv(join(fdir, 'trips.txt'))) {
      ...
      // feeds that ship no direction_id supply their own key (see MODES)
    ...
    // trim the shape to the passenger stretch: shapes that overshoot the
    // termini into depot access tracks lie outside the OSM passenger rails and
    // match as breaks and raw chords. Applies to the WKD, whose shapes run past
    // Grodzisk Radońska into the depot throat.
    for (const r of feedReps) {
  ...
  // Subway-only repair for cities whose OSM tunnels are mapped as per-direction
  // ways that meet nowhere. Never fires here — see the function's own note on
  // why plain rail must not be welded in this region.
  ...
  for (const r of reps) {
    ...
    if (r.pseudo) {
      ...
      opts = { gapMin: GAP_MIN };
      // Sibling cities widen the snap net here, because their metro shapes are
      // street-grid sketches of a tunnel. The WKD needs no such allowance: it
      // runs on the surface and its shapes follow the surveyed track, so the
      // tight default holds it on the right rails — which matters, since the
      // extract also carries PKP line 1 running parallel to the north.
    }
  ...
  for (const e of stopAgg.values()) {
    ...
    // somewhere else entirely (a feed that keeps stops on a road neither its
    // own geometry nor OSM knows anymore). A disc stranded in open country is
    // worse than a missing stop, so it is left out and logged.
    ...
    if (!isMetroStop && best && bestRun && bestRun.matchedXY[best.segIdx + 1]) {
      ...
      // both discs flipped the same way) — when the pole sits ON the axis its
      // coordinate carries no side signal; use the right-hand rule instead:
      // doors open to the RIGHT of the direction of travel. Offset poles keep
      // the data's side.
      // The cut-off is the uncertainty of the AXIS (OSM centerline vs. GPS),
      // not the width of the street: anything beyond it is a real kerb
      // position. The sibling cities inherited 6 m, tuned on wide urban
      // avenues; on this county network — rural roads, poles at the verge —
      // the offsets form one clean hump at 3–6 m (median 4.8 m), so 6 m
      // discarded a good side signal for 1265 of 1755 poles and left 15% of
      // the two-pole stops facing the same way (user report, Grodzisk Maz.
      // Wileńska: poles 4.8 m east and 3.0 m west of one centerline, both
      // drawn west). At 1.5 m only the 57 poles digitised onto the centerline
      // itself fall back, and same-facing pairs drop to 1%. Measured on the
      // 596 two-pole stops here, anything in 1–2 m scores the same (6–8 bad
      // pairs) — the value is not delicate, the old 6 m was simply too coarse.
      const AXIS_NOISE = 1.5;
      const offM = Math.abs(cross) / Math.hypot(dx, dy);
      const side = offM < AXIS_NOISE ? -1 : Math.sign(cross);
