...
async function init() {
  ...
  map.addControl(new maplibregl.AttributionControl({ compact: true, customAttribution: 'Timetables: GPA (Grodziskie Przewozy Autobusowe) · WKD GTFS' }));
  ...
  const nRail = meta.lines.filter((l) => l.mode === 'tram').length;
  document.getElementById('count').textContent = `(${nBus} bus · ${nRail} railway)`;
  ...
  const PALETTE = [
    ...
    // WKD operator purple (uppercase — exactly as the pipeline emits it from
    // routes.txt; a color missing here degrades to the darken() fallback icon,
    // so keep this in sync with the feed)
    ['#A518A3', '#5B0D5A'], // WKD
  ];
  ...
  async function exportBBox(bbox, btn, doneLabel, opts) {
    ...
    try {
      ...
      const txt = '© OpenStreetMap contributors · OpenFreeMap · GTFS: GPA (Grodziskie Przewozy Autobusowe), WKD';
      ...
      a.download = `grodzisk-transit_${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())}_${pad(d.getHours())}${pad(d.getMinutes())}_${out.width}x${out.height}.png`;
  ...
  btnAtlas.addEventListener('click', async () => {
    ...
    try {
      ...
      const a = document.createElement('a');
      a.download = `grodzisk-transit-giant_${d.getFullYear()}-${p2(d.getMonth() + 1)}-${p2(d.getDate())}_${Wf}x${Hf}.png`;
      a.href = URL.createObjectURL(blob);
  ...
  // NOT fitBounds(meta.bbox): the GPA network reaches Sochaczew, Mszczonów and
  // Łomianki, so fitting the data bbox would open on 50 km of countryside with
  // Grodzisk as a speck. Open on the town and its WKD spine instead.
  map.jumpTo({ center: [20.63, 52.11], zoom: 12.2 });
}
